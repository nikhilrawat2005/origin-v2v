    ...
    <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="space-y-6">
          ...
          {showAddForm && (
            <div className="bg-white border border-slate-100 p-6 rounded-3xl shadow-sm animate-in fade-in duration-200">
              <h3 className="font-bold text-slate-800 text-sm mb-4">Post New Discussion Thread</h3>
              <form onSubmit={handleCreatePost} className="space-y-4">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Thread Title</label>
                  <input
                    type="text"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Category Tag</label>
                    <select
                      value={postTag}
                  ...
                  <div className="flex items-center gap-2 pt-5">
                    <input
                      ...
                      className="rounded text-brand-purple focus:ring-brand-purple border-slate-200 w-4 h-4"
                    />
                    <label htmlFor="anon" className="text-xs font-semibold text-slate-650">
                      Post Anonymously
                    </label>
                  </div>
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Thread Content</label>
                  <textarea
                    rows={4}
          ...
          <div className="space-y-4">
            {filteredPosts.map((post) => (
              ...
              >
                ...
                <p className="text-slate-650 text-xs leading-relaxed whitespace-pre-wrap">{post.body}</p>

                {/* Actions Bar */}
                <div className="flex items-center gap-4 pt-2 border-t border-slate-50 text-xs text-slate-450 font-bold">
                  <button
                ...
                {expandedPostId === post.id && (
                  <div className="bg-slate-50 border border-slate-100 p-4 rounded-2xl space-y-4 animate-in slide-in-from-top duration-200">
                    <div className="space-y-3">
                      {(postReplies[post.id] || []).map((reply) => (
                        <div key={reply.id} className="bg-white border border-slate-100 p-3 rounded-xl flex gap-3">
                          <div className="w-7 h-7 rounded-full bg-purple-100 text-brand-purple flex items-center justify-center font-bold text-xs">
                            {reply.authorInitial}
                          </div>
                          <div className="flex-1 space-y-0.5">
                            ...
                            <p className="text-slate-700 text-xs font-medium">{reply.body}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 text-xs p-2.5 border border-slate-200 bg-white rounded-xl outline-none focus:border-brand-purple"
                      />
                      <button
            ...
            {filteredPosts.length === 0 && (
              <div className="text-center py-16 bg-white border border-slate-100 rounded-3xl">
                ...
                <h4 className="font-bold text-slate-805 text-sm">No discussion threads found</h4>
      ...
      {activeSubTab === "mentorship" && (
        <div className="space-y-6">
          ...
          {showMentorForm && (
            <div className="bg-white border border-slate-100 p-6 rounded-3xl shadow-sm animate-in fade-in duration-200 max-w-xl">
              <h3 className="font-bold text-slate-800 text-sm mb-4">Post Mentorship Request</h3>
              <form onSubmit={handleCreateMentorship} className="space-y-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Target Domain / Field</label>
                  <input
                    type="text"
                ...
                <div className="space-y-1 sm:col-span-2">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Specific Goals</label>
                  <input
                    type="text"
                ...
                <div className="sm:col-span-2 flex justify-end gap-2 pt-2">
                  <button
                    ...
                    className="px-4 py-2 border border-slate-200 text-slate-655 rounded-xl text-xs font-semibold hover:bg-slate-50"
                  >
                    Cancel
                  </button>
                  <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {mentorships.map((req) => (
              <div
                key={req.id}
                className="bg-white border border-slate-105 rounded-3xl p-6 shadow-sm hover:shadow-md transition-shadow space-y-4"
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-2.5">
                    <div className="p-2.5 bg-gradient-to-tr from-purple-50 to-indigo-50 text-brand-purple rounded-2xl border border-purple-100">
                      <Users className="w-5 h-5" />
                    </div>
                    <div>
                      <h4 className="font-extrabold text-slate-800 text-sm leading-snug">{req.name}</h4>
                ...
                <div className="space-y-2 border-t border-slate-50 pt-3">
                  <div>
                    <h5 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Goals</h5>
                    <p className="text-slate-650 text-xs font-medium">{req.goals || "Not specified"}</p>
                  </div>
                  <div>
                    <h5 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Availability</h5>
                    <p className="text-slate-650 text-xs font-medium">{req.availability || "Flexible"}</p>
                  </div>
                </div>

                <button
                  onClick={() => alert(`Connect request sent to ${req.name}! 🌸`)}
                  className="w-full py-2.5 border border-purple-100 bg-purple-50/50 hover:bg-brand-purple hover:text-white rounded-xl text-xs font-bold text-brand-purple transition-all shadow-sm flex items-center justify-center gap-1.5"
                >
            ...
            {mentorships.length === 0 && (
              <div className="col-span-2 text-center py-16 bg-white border border-slate-100 rounded-3xl">
                <Users className="w-12 h-12 text-slate-300 mx-auto mb-2" />
                <h4 className="font-bold text-slate-805 text-sm">No mentorship requests</h4>
                <p className="text-slate-450 text-xs mt-1">
